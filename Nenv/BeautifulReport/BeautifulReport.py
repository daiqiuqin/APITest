...
class ReportTestResult(unittest.TestResult):
    ...
    def __init__(self, suite, stream=sys.stdout):
        ...
        # 下面为daiqiuqin添加
        # 在项目初始化时加入logger,设置日志容器名称要与自己设置的一致
        self.loggor = logging.getLogger('dqq_apiLog')
    ...
    def startTest(self, test) -> None:
        ...
        #daiqiuqin添加
        #在stertTest函数中初始化logging Handler,记录到内存中
    ...
    def stopTest(self, test) -> None:
        ...
        # daiqiuqin添加
        # 每个用例执行完后，最好清除handler，在stopTest函数中加入
        # 清除log的handle----fancc
    ...
    def complete_output(self):
        ...
        #daiqiuqin添加，在complete_output函数的返回值中加入logging存在内存中的输出，用换行符隔开
