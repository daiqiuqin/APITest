...
@ddt
class test_InvoiceTopdf(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        #执行testCase前先清除环境变量
        clear_EnvData_attrs()
        log.info("test_InvoiceTopdf测试套件开始执行,共有{}条用例".format(maxRow))

    @classmethod
    def tearDownClass(cls) -> None:
        log.info("test_InvoiceTopdf测试套件执行完毕")
    ...
    @data(*alldatas)
    def test_InvoiceTopdf(self,data):
        # 判断是否跳过用例
        ...
        if extract_data:
            # 提取json返回数据存入全局变量
            fpgsdz=jsonpath.jsonpath(response_json,extract_data)[0]
            setattr(EnvData, "fpgsdz",fpgsdz)
            print("fpgsdz{}".format(fpgsdz))
