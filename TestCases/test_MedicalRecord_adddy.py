#-*-coding:utf-8 -*-
#@Time :2021/3/22 13:53
#@Author :daiqiuqin
#@File  :test_addMedicalRecord_dy.py
...
@ddt
class test_MedicalRecord_adddy(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        #执行testCase前先清除环境变量
        clear_EnvData_attrs()
        log.info("test_addMedicalRecord测试套件开始执行,共有{}条用例".format(maxRow))

    @classmethod
    def tearDownClass(cls) -> None:
        log.info("test_addMedicalRecord测试套件执行完毕")
    ...
    @data(*alldatas)
    def test_addMedicalRecord_dy(self,data):
        # 判断是否跳过用例
        ...
        try:
            ...
            try:
                Wexcel(output_file, readSheet, self.row, id, title, method, request_url, request_body, expected,
                       ...
                       testResult)
                # 写入的行数加1
                test_MedicalRecord_adddy.row += 1
            except IOError as e:
                print(e)
