...
#@Author :daiqiuqin
#@File  :test_MedicalRecord_cancel.py
...
@ddt
class test_MedicalRecord_cancel(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        #执行testCase前先清除环境变量
        clear_EnvData_attrs()
        log.info("test_MedicalRecord_cancel测试套件开始执行,共有{}条用例".format(maxRow))

    @classmethod
    def tearDownClass(cls) -> None:
        log.info("test_MedicalRecord_cancel测试套件执行完毕")
        ...
        try:
            ...
            try:
                Wexcel(output_file, readSheet, self.row, id, title, method, request_url, request_body, expected,
                       ...
                       testResult)
                # 写入的行数加1
                test_MedicalRecord_cancel.row += 1
            except IOError as e:
                print(e)
