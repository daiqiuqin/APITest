#-*-coding:utf-8 -*-
#@Time :2021/4/15 15:33
#@Author :daiqiuqin
#@File  :test_MedicalRecord_cj_cancel.py
...
@ddt
class test_MedicalRecord_cj_cancel(unittest.TestCase):
    @classmethod
    ...
    @data(*alldatas)
    def test_MedicalRecord_cj_cancel(self,data):
        # 判断是否跳过用例
        ...
        request_body = type(response.request.body)
        if not isinstance(type(request_body),str):
            request_body="requestbody非str格式"
        ...
        try:
            ...
            try:
                Wexcel(output_file, readSheet, self.row, id, title, method, request_url, request_body, expected,
                       ...
                       testResult)
                # 写入的行数加1
                test_MedicalRecord_cj_cancel.row += 1
            except IOError as e:
                print(e)
