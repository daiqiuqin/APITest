...
#@Author :daiqiuqin
#@File  :test_MedicalRecord_sendConfirm.py
...
@ddt
class test_MedicalRecord_sendConfirm(unittest.TestCase):
    @classmethod
    ...
    @data(*alldatas)
    def test_cancelMedicalRecord(self,data):
        # 判断是否跳过用例
        ...
        try:
            ...
            try:
                Wexcel(output_file, readSheet, self.row, id, title, method, request_url, request_body, expected,
                       ...
                       testResult)
                # 写入的行数加1
                test_MedicalRecord_sendConfirm.row += 1
            except IOError as e:
                print(e)
